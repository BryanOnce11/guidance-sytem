...
@section('content')
    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="flex flex-wrap items-center col-span-12 mt-2 intro-y sm:flex-nowrap">
            ...
            <div class="hidden mx-auto text-slate-500 md:block">
                Showing {{ $good_moral_pendings->firstItem() }} to {{ $good_moral_pendings->lastItem() }} of
                {{ $good_moral_pendings->total() }} entries
            </div>
        ...
        <div class="col-span-12 overflow-auto intro-y lg:overflow-visible">
            <table data-tw-merge="" class="w-full text-left -mt-2 border-separate border-spacing-y-[10px]">
                ...
                <tbody>
                    @forelse ($good_moral_pendings as $good_moral_pending)
                        <tr data-tw-merge="" class="intro-x">
                            <td data-tw-merge=""
                                ...
                                {{ $good_moral_pending->student->student_id }}
                            </td>
                            <td data-tw-merge=""
                                ...
                                <a class="font-medium whitespace-nowrap" href="">
                                    {{ $good_moral_pending->student->fname }} {{ $good_moral_pending->student->m_i }}
                                    {{ $good_moral_pending->student->lname }}
                                </a>
                                <div class="mt-0.5 whitespace-nowrap text-xs text-slate-500">
                                    {{ $good_moral_pending->student->gender }}
                                </div>
                            </td>
                            <td data-tw-merge=""
                                class="px-5 py-3 border-b dark:border-darkmode-300 box rounded-l-none rounded-r-none border-x-0 text-center shadow-[5px_3px_5px_#00000005] first:rounded-l-[0.6rem] first:border-l last:rounded-r-[0.6rem] last:border-r dark:bg-darkmode-600">
                                {{ $good_moral_pending->student->course->code }}
                            </td>
                            <td data-tw-merge=""
                                class="px-5 py-3 border-b dark:border-darkmode-300 box rounded-l-none rounded-r-none border-x-0 text-center shadow-[5px_3px_5px_#00000005] first:rounded-l-[0.6rem] first:border-l last:rounded-r-[0.6rem] last:border-r dark:bg-darkmode-600">
                                {{ $good_moral_pending->student->year_lvl }}
                            </td>
                            <td data-tw-merge=""
                                class="px-5 py-3 border-b dark:border-darkmode-300 box rounded-l-none rounded-r-none border-x-0 text-center shadow-[5px_3px_5px_#00000005] first:rounded-l-[0.6rem] first:border-l last:rounded-r-[0.6rem] last:border-r dark:bg-darkmode-600">
                                {{ $good_moral_pending->date_requested->setTimezone('Asia/Manila')->format('M d, Y') }}
                            </td>
                            <td data-tw-merge=""
                            ...
                            <td data-tw-merge=""
                                ...
                                <div class="flex items-center justify-center">
                                    <a class="flex items-center mr-3 text-primary"
                                        href="{{ route('admin.good-moral.letter', $good_moral_pending) }}">
                                    ...
                                    <a class="flex items-center mr-3 text-success" data-tw-toggle="modal"
                                        data-tw-target="#new-order-modal{{ $good_moral_pending->id }}" href="#">
                                        <i data-tw-merge="" data-lucide="thumbs-up" class="stroke-1.5 mr-1 h-4 w-4"></i>
                                        Approved
                                    </a>
                                    <a class="flex items-center text-danger" data-tw-toggle="modal"
                                        data-tw-target="#reject-confirmation-modal{{ $good_moral_pending->id }}"
                                        href="#">
        ...
        <div class="flex flex-wrap items-center col-span-12 intro-y sm:flex-row sm:flex-nowrap">
            <nav class="w-full sm:mr-auto sm:w-auto">
                {{ $good_moral_pendings->links('vendor.pagination.tailwind') }}
            </nav>
    ...
    @foreach ($good_moral_pendings as $good_moral_pending)
        <!-- BEGIN: New Order Modal -->
        <div data-tw-backdrop="" aria-hidden="true" tabindex="-1" id="new-order-modal{{ $good_moral_pending->id }}"
            class="modal group bg-black/60 transition-[visibility,opacity] w-screen h-screen fixed left-0 top-0 [&:not(.show)]:duration-[0s,0.2s] [&:not(.show)]:delay-[0.2s,0s] [&:not(.show)]:invisible [&:not(.show)]:opacity-0 [&.show]:visible [&.show]:opacity-100 [&.show]:duration-[0s,0.4s]">
            <div data-tw-merge=""
                class="w-[90%] mx-auto bg-white relative rounded-md shadow-md transition-[margin-top,transform] duration-[0.4s,0.3s] -mt-16 group-[.show]:mt-16 group-[.modal-static]:scale-[1.05] dark:bg-darkmode-600 sm:w-[460px]">
                <div class="flex items-center px-5 py-3 border-b border-slate-200/60 dark:border-darkmode-400">
                    <h2 class="mr-auto text-base font-medium">Approved Good Moral</h2>
                </div>
                <form action="{{ route('admin.good-moral.approved', $good_moral_pending) }}" method="POST">
                    @csrf
                    @method('PUT')
    ...
    @foreach ($good_moral_pendings as $good_moral_pending)
        ...
        <div data-tw-backdrop="" aria-hidden="true" tabindex="-1"
            id="reject-confirmation-modal{{ $good_moral_pending->id }}"
            class="modal group bg-black/60 transition-[visibility,opacity] w-screen h-screen fixed left-0 top-0 [&:not(.show)]:duration-[0s,0.2s] [&:not(.show)]:delay-[0.2s,0s] [&:not(.show)]:invisible [&:not(.show)]:opacity-0 [&.show]:visible [&.show]:opacity-100 [&.show]:duration-[0s,0.4s]">
            <div data-tw-merge=""
                class="w-[90%] mx-auto bg-white relative rounded-md shadow-md transition-[margin-top,transform] duration-[0.4s,0.3s] -mt-16 group-[.show]:mt-16 group-[.modal-static]:scale-[1.05] dark:bg-darkmode-600 sm:w-[460px]">
                <div class="p-5 text-center">
                    ...
                    <div class="mt-5 text-3xl">Are you sure?</div>
                    <div class="mt-2 text-slate-500">
                        Do you really want to reject the Good Moral request of this student? <br>
                        This process cannot be undone.
                    </div>
                </div>
                <form action="{{ route('admin.good-moral.reject', $good_moral_pending) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="px-5 pb-8 text-center">
                        <button data-tw-merge="" data-tw-dismiss="modal" type="button"
